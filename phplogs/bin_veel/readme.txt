...
kopeerimise script copylogs.php. Sellest täpsemalt ja kuidas see toimib.
	...
	e.Tõstetakse sisse tõmmatud failid processed kataloogi.
	  ...
	  Enamasti on need väljad puudu veateadete ridadel, on ka muid
	  üksikuid juhtumeid.
	f.scripti lõpp. -> töötab tavaliselt loetud sekunditega (alla 1min). 
...
arhiveerimise script. Sellest täpsemalt ja kuidas see toimib.
	...
	d.arhiveeritakse
	  Kui rakendus on ainult ühel nodel, siis liidetakse tunnifailid lihtsalt kokku.
	  Muidu tehakse kordus üle tundide, kus
	    ...
	    tulemus faili ning alustatakse uue tunniga samamoodi.
	  Ühe päeva tulemusfail pakitakse kokku (gzip).
	...
	f.scripti lõpp. - töötab tavaliselt 3,5-5,5tunni vahel (oleneb faili suurustest), 
	  1x päevas, öösel.
