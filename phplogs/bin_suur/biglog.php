...
include ('/home/webapp/phplogs/bin_suur/conf/hosts_conf.php');
#include ('/home/webapp/phplogs/bin_veel/conf.php');
include ('/home/webapp/phplogs/bin_suur/config.class.php');
...
#is_archived($filename, $config);
		...
		echo "copy_logs, foreach, fail= ".$filename." not_excluded->".not_excluded($filename, $config)."\n";
        ...
        $filename = check_filename($filename, $host, $config);
			...
			if (!is_gzip($filename, $config)) {
				...
				echo "nit gz, Filename3= $filename \n ";
			}
	...
	echo "is_repl_gz kontroll nüyyd, filen= $filen \n ";
...
if (not_processed($filename, $config) ) {
        $filen=$filename;
        if (is_gzip($filename, $config)) {
                echo "Pakitakse lahti käsuga= gunzip -f ".$config->config_values[paths][incoming_path]."$filename \n";
                $result1 = exec("gunzip -f ".$config->config_values[paths][incoming_path].$filename, $out1);
...
if (is_incoming($filename, $config) ) {
        $filen=$filename;
        if (is_gzip($filename, $config)) {
                echo "Pakitakse lahti käsuga= gunzip -f ".$config->config_values[paths][incoming_path]."$filename \n";
                $result1 = exec("gunzip -f ".$config->config_values[paths][incoming_path].$filename, $out1);
...
function not_processed($filename, $config) {
        $pos = strpos($filename, "-");
        $posr = strrpos($filename, "-");
        $name = substr($filename, $pos+1, $posr-$pos-1);
        echo "not_processed, nimiiiiiiiiiii= $name ja $name".".gz ja filename= $filename\n";
        if (is_file($config->config_values[paths][processed_path].$name) or is_file($config->config_values[paths][processed_path].$name.".gz")) {
        #       echo "protsessidtud \n";
                return FALSE;
        }
        #echo "Ei ole protsessitud \n";
        return TRUE;
}


function is_incoming($filename, $config) {
        $pos = strpos($filename, "-");
        $posr = strrpos($filename, "-");
        $name = substr($filename, $pos+1, $posr-$pos-1);
        echo "not_processed, nimiiiiiiiiiii= $name ja $name".".gz ja filename= $filename\n";
        if (is_file($config->config_values[paths][incoming_path].$name) or is_file($config->config_values[paths][incoming_path].$name.".gz")) {
        #       echo "protsessimata aga incoming kaustas \n";
                return TRUE;
        }
        #echo "Ei ole protsessitud aga in incoming kaustas \n";
        return FALSE;
}
