...
#var_dump($inc_files);
#$coun1=count($inc_files);
#$coun2=sizeof($inc_files);
...
#is_archived($filename, $config);
        ...
        //filename kontroll ja muutmine reaalseks:
        $file1 = $filename;
        $file2 = $filename;
        $name2 = explode('.', $file2);

        //kui punkte on rohkem, siis teeme väikese muudatuse
        $ccc = substr_count($name2[0], '-');
        if ($ccc==1) {
               $rak = explode('-',$name2[0]);
               $name2[0] = $rak[0]."_".$rak[1];
                #."-".$rak[2];
               $rak = explode('-',$name2[0]);
               $rakendus2 = $rak[1];
               echo "failinimi vigane1, ccc==1, file2=".$file2."  name0=".$name[0]." \n";
        } else echo "failinimi pole vigane1, rakendus2 e name2=".$file2."  ccc=".$ccc."\n";

        $cc = substr_count($file2, '.');
        if ($cc==4) {
               $file2=$name2[0]."_".$name2[1]."_".$name2[2].".".$name2[3].".".$name2[4];
               echo "failinimi vigane2, vana nimi=".$filename."  uus nimi=".$file2."\n";
               $name = explode('.', $file2);
                $filename = $file2;
        }
        if ($cc==5) {
               $file2=$name2[0]."_".$name2[1]."_".$name2[2]."_".$name2[3].".".$name2[4].".".$name2[5];
               echo "failinimi vigane3, vana nimi=".$filename."  uus nimi=".$file2."\n";
               $name2 = explode('.', $file2);
                $filename = $file2;
        }

        echo "Töödeldud failinimi=".$file2."  vana failini oli: ".$file1." \n";


			if (!is_copied($filename, $files)) {
				#and !is_archived($filename, $config) ) {
				#)
				echo "esialgne failinimi=$filename, \n";
...
#               $rak = explode('-',$name2[0]);
#               $rakendus2 = $rak[1];
#               echo "ccc==1, rakendus2=".$rakendus2."  name0=".$name[0]." \n";
#        } else echo "rakendus2 e name0=".$name[0]."  ccc=".$ccc."\n";

#        $cc = substr_count($file2, '.');
#        if ($cc==4) {
#               $file2=$name2[0]."_".$name2[1]."_".$name2[2].".".$name2[3].".".$name2[4];
#               echo "failinimi vigane, vana nimi=".$filename."  uus nimi=".$file2."\n";
#               $name = explode('.', $file2);
#		$filename = $file2;
#        }
#        if ($cc==5) {
...
#	echo "Töödeldud failinimi=".$file2." \n";
...
function is_gzip($filename, $config) {
        ...
        foreach ($gzipped as $name) {
                if (substr_count($filename, $name) > 0 ) {
			echo "is_gzip= filename: $filename  ja  name: $name  is_gzip=true \n";
                        return TRUE;
                }
        }
	echo "is_gzip= filename: $filename  ja  name: $name  is_gzip=false \n";
        return FALSE;
}
...
function is_repl_gz($filename, $config) {
        ...
        foreach ($gzipped as $name) {
                if (substr_count($filename, $name) > 0 ) {
                        echo "is_repl_gzip= filename: $filename  ja  name: $name  is_repl_gz=TRUE  \n";
                        return TRUE;
                }
        }
	echo "is_repl_gzip= filename: $filename  ja  name: $name  is_repl_gz=FALSE  \n";
        return FALSE;
}
...
function send_mail_bak($config, $filename, $file_size) {
...
if ($file_size > $config->config_values[errors][max_filesize]) {
	...
	$mail->addAddress($config->config_values[errors][error_mail]);     // Add a recipient
        #$mail_body = sprintf($config->config_values[errors][max_filesize_message],$filename,formatBytes($config->config_values[errors][max_filesize]),formatBytes($file_size)); //mail body
	$mail->Body    = sprintf($config->config_values[errors][max_filesize_message],$filename,formatBytes($config->config_values[errors][max_filesize]),formatBytes($file_size)); //mail body
        #$subject = "Veateade XGLOGS masinast"; //subject
	$mail->Subject = "Veateade XGLOGS masinast";
} else {
        #$recipient = $config->config_values[errors][warning_mail];
	$mail->addAddress($config->config_values[errors][warning_mail]);
        #$mail_body = sprintf($config->config_values[errors][max_filesize_warningmessage],$filename,formatBytes($config->config_values[errors][max_filesize_warning]),formatBytes($file_size)); //mail body
	$mail->Body    = sprintf($config->config_values[errors][max_filesize_warningmessage],$filename,formatBytes($config->config_values[errors][max_filesize_warning]),formatBytes($file_size)); //mail body
        #$subject = "Hoiatus XGLOGS masinast"; //subject
	 $mail->Subject = "Hoiatus XGLOGS masinast"; //subject
}
#$header = "From: ". $Name . " <" . $email . ">\r\n"; //optional headerfields
#mail($recipient, $subject, $mail_body, $header); //mail command :)
...
function send_error_bak($config, $server) {
        #/bin/echo "Hoiatus: esines OutOfMemory viga KREEDO LIVE nodeA logis (tasuks moelda kreedowli1 restardile)" | /bin/mail valve@sorts.emt.ee
...
if ($file_size > $config->config_values[errors][max_filesize]) {
        #$recipient = $config->config_values[errors][error_mail];
	$mail->addAddress($config->config_values[errors][error_mail]);
 	#$mail_body = sprintf($config->config_values[errors][error_server_message],$server); //mail body
	$mail->Body    = sprintf($config->config_values[errors][error_server_message],$server); //mail body
        #$subject = "Veateade XGLOGS masinast"; //subject
