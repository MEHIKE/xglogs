	...
	while (($file = readdir($open_path)) !== false) {
		if (is_file($path.'/'.$file)) {
			...
			//kui punkte on rohkem, siis teeme väikese muudatuse
		        $cc = substr_count($file, '.');
			echo "cc==".$cc."  rakendus=".$file." \n";
	        	if ($cc==4) {
				$file=$name[0]."_".$name[1]."_".$name[2].".".$name[3].".".$name[4];
				echo "failinimi vigane, uus nimi=".$file."\n";
				$name = explode('.', $file);	
		        }
                        if ($cc==5) {
                                $file=$name[0]."_".$name[1]."_".$name[2]."_".$name[3].".".$name[4].".".$name[5];
                                echo "failinimi vigane, uus nimi=".$file."\n";
                                $name = explode('.', $file);
                        }
			if ($cc==2) {
				//echo "cc==2  rakendus=".$file." \n";
				if (substr_count($file, 'msp_energia')) {
					$file = str_replace("msp_energia.","msp_energia_",$file);
					//$fname = str_replace(".","_",$fname);
					echo "cc=2 msp_energia. on asendatud msp_energia_ \n";
					$name = explode('.', $file);
				}
			}
                        if ($cc==3) {
                                ...
                                if (substr_count($file, 'msp_energia')) {
                                        $file = str_replace("msp_energia.","msp_energia_",$file);
                                        //$fname = str_replace(".","_",$fname);
                                        echo "cc=3 msp_energia. on asendatud msp_energia_ \n";
                                        $name = explode('.', $file);
                                }
                        }


			$cc = substr_count($name[0], '-');
			
			if ($cc==2) {
	                        $rak = explode('-',$name[0]);
				$rak = $rak[0]."_".$rak[1]."-".$rak[2];
				$rak = explode('-',$rak);
        	                $rakendus = $rak[1];
				echo "rakendus=".$rakendus." \n";
			}
			else {
				$rak = explode('-',$name[0]);
				$rakendus = $rak[1];
				echo "rakendus2=".$rakendus."  cc=".$cc." \n";
			}
			$paev = substr($name[2], 0, 10);
			$tund = substr($name[2], 11, 2);
			$host = $rak[0];
			$hour_files = $files[$rakendus][$paev];
			$hour_files[$tund][$host] = $file1;
