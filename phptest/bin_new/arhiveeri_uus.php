...
foreach($files as $rakendus => $paevad )
{
	...
	foreach($paevad as $paev => $tunnid )
	{
		...
		if ($paev != $tana) {
			echo "Päevad: $paev -> $tunnid Kell: ".date("d-F-Y H:i:s",strtotime("now"))."\n";
...
function arhiveeri($files, $rakendus, $paev, $config) {
	...
	foreach($files as $tund => $file ) {
		...
		foreach($file as $host => $filename) {		
			...
			if ($failid[$filename][handle]) {
				if (!feof($failid[$filename][handle])) {
					...
					$failid = get_lines($failid, $filename, $read_bytes);
					$failid[$fn][lines] = array_shift($failid[$fn][lines]);
		...
		while ($veel) {
			...
			#Kui selle failiga on lõpp, siis seda ei vaata
			if ($failid[$filename][line] != 'END_OF_FILE') {
				if ($failid[$fn][last][0] == TRUE) {
					$failid = get_lines($failid, $fn, $read_bytes);
					$failid[$fn][lines] = array_shift($failid[$fn][lines]);
				} else { #kui read said otsa, sisi loeb uue portsu sisse
					# aga siin kustutab esimese rea arrayst, saab uueesimese
					$failid[$fn][lines] = array_shift($failid[$fn][lines]);
				}
			}
...
function get_lines($failid, $fn, $read_bytes) {
	for($i = 0, $i<100, $i++) {
		if (!feof($failid[$fn][handle])) {
			$failid[$fn][line][$i] = stream_get_line($failid[$fn][handle], $read_bytes, "\n");
                        $failid[$fn][time][$i] = find_time_from_line($failid[$fn][line][$i], $paev);
                        if ($failid[$fn][time][$i] <= null_time()) {
	                        $failid[$fn][time][$i] = $aeg;
                                $failid[$fn][line][$i] = print_time($aeg).'|'.$failid[$fn][line][$i];
                        }
                        $aeg = $failid[$fn][time][$i];
			$failid[$fn][last][$i] = FALSE;
                } else {
                        $failid[$fn][line][$i] = "END_OF_FILE";
                        $failid[$fn][time][$i] = null_time();
			$failid[$fn][last][$i] = TRUE;
                        $aeg=null_time();
			break;
                }

	}
	$failid[$fn][last][99] = TRUE;
	return $failid;
}
