...
function arhiveeri($files, $rakendus, $paev, $config) {
	...
	$null_time = null_time();
        ...
        if (is_one_node($files)) {
        	...
        	if (is_warning_size($files, $config) or is_mdp($rakendus)) {
...
#	if ($summ >= ($config->config_values[errors][max_filesize_warning])) {
#		echo "failisuuruste summa ületab warning piiri: ".$summ." >= ".$config->config_values[errors][max_filesize_warning]."  \n";
#		return TRUE;
#	}
        #if (count > 1) {
        #        return FALSE;
